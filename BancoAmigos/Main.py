...
class MenuPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tabla_button=tk.Button(button_frame,text='ESTADO DE CUENTA',font=('orbitron',13),command=lista,relief='raised',borderwidth=3,width=30,height=4)
        tabla_button.grid(row=3,column=0,pady=5)
        ...
        trasf_button=tk.Button(button_frame,text='ACERCA DE NOSOTROS',font=('orbitron',13),command=about_us,relief='raised',borderwidth=3,width=30,height=4)
        trasf_button.grid(row=3,column=1,pady=5)
...
class About(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        heading=tk.Label(self,text='Acerca de',font=('orbitron',45,'bold'),foreground='white',background='#1e1e2d')
        heading.pack(pady=25)
        main_menu_label=tk.Label(self,text='DESARROLLADORES',font=('orbitron',20,'bold'),fg='white',bg='#1e1e2d')
        ...
        tk.Label(upperframe, text=f'Desarrolladores Front-End: ', font=('orbitron',16),fg='yellow', bg='#1e1e2d').pack(pady=5)
        tk.Label(upperframe, text= f'Jesus Aaron Dominguez Rodriguez',font=('orbitron',16,'bold'),fg='white',bg='#1e1e2d').pack(pady=5)
        tk.Label(upperframe, text= f'Antonio Alberto Noh Campos',font=('orbitron',16,'bold'),fg='white',bg='#1e1e2d').pack(pady=5)

        tk.Label(upperframe, text=f'Desarrolladores Back-End:', font=('orbitron',16),fg='yellow', bg='#1e1e2d').pack(pady=5)
        tk.Label(upperframe, text=f'Alexis Omar Galvan Lozano',font=('orbitron',16,'bold'),fg='white',bg='#1e1e2d').pack(pady=5)
        tk.Label(upperframe, text=f'David Fabrizio Recillas Abarca',font=('orbitron',16,'bold'),fg='white',bg='#1e1e2d').pack(pady=5)

        tk.Label(upperframe, text=f'Desarrollo Base de Datos:', font=('orbitron',16),fg='yellow', bg='#1e1e2d').pack(pady=5)
        tk.Label(upperframe, text=f'Guadalupe Quiroz Quiroz',font=('orbitron',16,'bold'),fg='white',bg='#1e1e2d').pack(pady=5)
    ...
    def __init__(self, parent, controller):
            ...
            def transfer(_):
                ...
                try:
                    ...
                    if int(deposito.get())>current_balance1:
                        ...
                        current_balance1 -= int(deposito.get())
                        curr2=str(val)
                        controller.shared_data['Balance'].set(current_balance1)
                        ...
                        mycursor.execute(f"update usuario set balance ={current_balance1} where accit = {username1} ")
                    ...
                    new_balance3=new_balance_1
                
                    new_balance3 += int(deposito.get())
                    messagebox.showinfo('TRANSFERENCIA','Transferencia exitosa!')
                    username2 = transferencia.get()
                    mydb=pyodbc.connect(driver='{SQL Server}', server='10.10.58.33', database='Gangrena', trusted_Connection='yes')
                    mycursor=mydb.cursor()
                    mycursor.execute("use Gangrena")
                    mycursor.execute(f"update usuario set balance ={new_balance3} where accit = {username2} ")
                    mydb.commit()
                    controller.show_frame('MenuPage')
...
class TablaPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.controller.title('BANCO AMIGOS')
        self.controller.state('zoomed')
        heading=tk.Label(self,text='ESTADO DE CUENTA',font=('orbitron',45,'bold'),foreground='white',background='#1e1e2d')
        heading.pack(pady=25)
        ##choose_amount_label=tk.Label(self,text='ESTADO DE CUENTA',font=('orbitron',17,'bold'),fg='white',bg='#1e1e2d')
        ...
        estiloTabla.configure("Treeview", font = ("Helvetica", 10, 'bold'), foreground = 'white', background = "#1e1e2d")
        estiloTabla.map('Treeview',background=[('selected', 'DarkOrchid1')], foreground=[('selected','black')] )
        estiloTabla.configure('Heading',background = 'white', foreground='navy',padding=3, font= ('Arial', 10, 'bold'))

        tabla=ttk.Treeview(upperframe,columns=("#0","#1","#2","#3"))
        ##tabla.grid(row=5,column=6,columnspan=3)
        tabla.heading("#0",text="Folio",anchor=CENTER)
        tabla.heading("#1",text="Fecha",anchor=CENTER)
        tabla.heading("#2",text="Saldo",anchor=CENTER)
        tabla.heading("#3",text="Operacion",anchor=CENTER)
        tabla.pack()
        ...
        def listar():
            ...
            for (folio,fecha,saldo,operacion) in respuestas:
                    tabla.insert('','0',text=folio,values=(fecha,saldo,operacion))
        def delete():
        ...
        boton2=ttk.Button(button_frame, text="Regresar", command=lambda:[delete(),quit()]).pack()
        ##boton2.grid(column=1, row=0, padx=4, pady=4)

        boton1=ttk.Button(button_frame, text="Listado completo", command=listar).pack()
        ##boton1.grid(column=2, row=0, padx=4, pady=4)
...
def register():
  ...
  label = Label(screen1, text = "Nombre(s)",font = ("orbitron", 14),bg='#1e1e2d',fg='white')
  label.place(x=370, y=270)
  name_entry = Entry(screen1,font = ("orbitron",14), textvariable = name,bg='white', justify = "center")
  name_entry.place(x=370, y=300)
 

  label = Label(screen1, text = "Tu numero de cuenta es:",font = ("orbitron", 14),bg='#1e1e2d',fg='white')
  label.place(x=720, y=470)
  rand=random.randint(1,100000)
  username=Label(screen1, text = rand,font = ("orbitron", 14,'bold'),bg='#1e1e2d', fg='white')
  username.place(x=720, y=490)

  label = Label(screen1, text = "Apellido Paterno",font = ("orbitron", 14),bg='#1e1e2d',fg='white')
  label.place(x=370, y=340)
  apellP_entry =  Entry(screen1,font = ("orbitron",14), textvariable = apellidoP,bg='white', justify = "center")
  apellP_entry.place(x=370, y=370)
  #apellP_entry.config(fg='black')
  #apellP_entry.pack()

  label = Label(screen1, text = "Apellido Materno",font = ("orbitron", 14),bg='#1e1e2d',fg='white')
  label.place(x=370, y=410)
  apellM_entry =  Entry(screen1,font = ("orbitron",14), textvariable = apellidoM,bg='white', justify = "center")
  apellM_entry.place(x=370, y=440)
  #apellM_entry.config(fg='black')
  #apellM_entry.pack()
  #Label(screen1, text = "",bg='#1e1e2d').pack()
  def generar():
        label = Label(screen1, text = "Tu correo es:",bg='#1e1e2d', fg='white',font = ("orbitron", 10))
        ...
        if apellidoM.get()=="" or apellidoP.get()=="" or name.get()=="":
            ...
            label = Label(screen1, text = Correo_info,bg='#1e1e2d',fg='white',font = ("orbitron", 10, 'bold'))
            label.place(x= 397, y=540)
  
  button1= tk.Button(screen1,text = "Generar correo",fg='#663259', bg = "#c2d5e3", width = "16", font = ("orbitron", 12, 'bold'),command=generar)
  button1.place(x= 397, y=480)

  label = Label(screen1, text = "Telefono",font = ("orbitron", 14),bg='#1e1e2d',fg='white')
  label.place(x=720, y=270)
  tel_entry =  Entry(screen1,font = ("orbitron",14), textvariable = telf,bg='white', justify = "center")
  #tel_entry.config(fg='black')
  tel_entry.place(x=720, y=300)

  label = Label(screen1, text = "Contraseña",font = ("orbitron", 14),bg='#1e1e2d',fg='white')
  label.place(x=720, y=340)
  password_entry =  Entry(screen1,font = ("orbitron",14), textvariable = password,bg='white', justify = "center")
  password_entry.config(show='●')
  password_entry.place(x=720, y=370)

  languages = ('Gerente', 'Supervisor', 'Cajero')
  label = Label(screen1,  text='Seleciona tu Posición', font= ("orbitron", 14), bg='#1e1e2d',fg='white')
  label.place(x=720, y=410)

    # option menu
  option_menu = ttk.OptionMenu(screen1, option_var,languages[0],*languages)
  option_menu.place(x=720, y=440)
  button_reg = Button(screen1,text = "REGISTRARSE",fg='white', bg = "#663259", width = "21", height = "2", font = ("orbitron", 12,'bold'),command=register_user)
...
def login():
  ...
  label1 = Label(screen2, text = "Bienvenido a",bg='#1e1e2d',fg='white',font = ("orbitron", 20))
  label1.place(x=330, y=245)

  #Label(screen2, text = "",bg='#1e1e2d').pack()

  #label2 = Label(screen2, text = "Banco Amigos",bg='#1e1e2d',fg='white',font = ("orbitron", 25,'bold'))
  #label2.place(x=620, y=295)
  ...
  label = ttk.Label(screen2, text = "Numero de cuenta",font = ("orbitron", 14), background = '#1e1e2d', foreground="white")
  label.place(x=900, y=255)
  #Label(screen2, text = "",bg='#1e1e2d').pack()
  username_entry1 = ttk.Entry(screen2,font = ("orbitron",14) ,textvariable = username_verify, justify = "center")
  username_entry1.place(x=860, y=290)
  #username_entry1.pack()

  #Label(screen2, text = "",bg='#1e1e2d').pack()
  label = ttk.Label(screen2, text = "Contraseña",font = ("orbitron", 14), background = '#1e1e2d', foreground="white")
  label.place(x=920, y=320)
  #Label(screen2, text = "",bg='#1e1e2d').pack()
  password_entry1 = ttk.Entry(screen2,font = ("orbitron",14), textvariable = password_verify, justify = "center")
  password_entry1.config(show='●')
  password_entry1.place(x=860, y=350)
  #password_entry1.pack()
  #Label(screen2, text = "",bg='#1e1e2d').pack()
  button_log = tk.Button(screen2,text = "Iniciar Sesión", background = '#663259', foreground = '#c2d5e3', width = "20", font = ("orbitron",14),command=login_verify)
